...
derive_pll_clocks 
create_generated_clock -name sd1clk_pin -source [get_pins {pll_1|altpll_component|auto_generated|pll1|clk[2]}] [get_ports {sdram_clk_o}]
create_generated_clock -name sysclk -source [get_pins {pll_1|altpll_component|auto_generated|pll1|clk[1]}]
...
set_input_delay -clock sd1clk_pin -max 5.8 [get_ports sdram_data*]
set_input_delay -clock sd1clk_pin -min 3.2 [get_ports sdram_data*]

# Delays for async signals - not necessary, but might as well avoid
# having unconstrained ports in the design
set_input_delay -clock sysclk -min 0.0 [get_ports {uart_rx_i}]
set_input_delay -clock sysclk -max 0.0 [get_ports {uart_rx_i}]
...
# Delays for async signals - not necessary, but might as well avoid
# having unconstrained ports in the design
#set_output_delay -clock sysclk -min 0.0 [get_ports uart_tx_o]
#set_output_delay -clock sysclk -max 0.0 [get_ports uart_tx_o]
...
set_false_path -from {*uart|txd} -to {uart_tx_o}
...
set_multicycle_path -from [get_clocks {sd1clk_pin}] -to [get_clocks {pll_1|altpll_component|auto_generated|pll1|clk[1]}] -setup -end 2
